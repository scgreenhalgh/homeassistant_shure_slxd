...
from .exceptions import SlxdConnectionError, SlxdProtocolError, SlxdTimeoutError
...
class SlxdClient:
    ...
    async def _send_command_multi_response(
        self, command: str, expected_count: int, timeout: float = DEFAULT_COMMAND_TIMEOUT
    ) -> list[ParsedResponse]:
        """Send command and receive multiple responses.

        Some SLX-D commands (like GET RSSI) return multiple REP messages.
        This method reads the expected number of responses.
        ...
        Returns:
            List of parsed responses from device
        ...
        for _ in range(expected_count):
            try:
                ...
                # If we got at least one response, return what we have
                if responses:
                    break
                raise SlxdTimeoutError(f"Command timed out after {timeout}s") from err
            except asyncio.IncompleteReadError as err:
